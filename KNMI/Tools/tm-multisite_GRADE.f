...
cJB	character*(10000) regel 
	...
	character*(5) sarg			! temporary command line argument
cJB	character*(*) fmt
...
cJB	integer cyear_obs, shift_y
	...
	! CHECK NUMBER OF CMDL ARGS 
	if(iarg().lt.3) then 
cJB	   write(0,*) 'syntax : tm-trans-month <scenario> <year> 
cJB  &                                          <number of series>'
	   write(0,*) 'syntax : tm-multisite <scenario> <year> 
     &                                          <number of series>'
	   write(0,*) 'Transforms daily precipitation with changes'
	   write(0,*) 'representative of the given year.'
	...
	open(22,
     &	   file='tm_'//scenario(1:instr(scenario,' ')-1)//'.txt',
     &	   status='OLD')
	call getarg(2,sarg)
	read(sarg,*) cyear		! read integer year

	call getarg(3,sarg)
	read(sarg,*) ts			! number of temperature series

C	write(*,'(a,i8)') '# tm-trans-month: SCENARIO='//
	write(0,'(a,i8)') '# tm-multisite: SCENARIO='//
     &	            scenario(1:instr(scenario,' ')-1)//
     &	           ',    YEAR =',cyear

	do while(.True.)
 222	   read(22,'(a1000)',end=122) regel 
