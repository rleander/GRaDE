       program thwt_general !with several changes by RL 201712-201801 (NG-Vecht); with "workaround" for EObs v19e (JB20190430)  
       ...
       ! gfortran -o thwt_EOBS_v15run_EOBSv19e_temp.log_test thwt-EOBS_v15.0_Rhine.o -L/usr/lib/x86_64-linux-gnu -lnetcdff
       ...
       integer                                        :: field_varid
       integer, dimension(3)                          :: field_dimids
       integer, dimension(3)                          :: field_dimlen
       integer                                        :: ngd1,ngd2,ntime                 ! size n of Grid Dimension 1 and 2 and time  
       integer, dimension(3)                          :: start,cnt
       real(kind=8),  dimension(:),   allocatable     :: lats
       real(kind=8),  dimension(:),   allocatable     :: lons
       real(kind=8),  dimension(:),   allocatable     :: lat_reg
       real(kind=8),  dimension(:),   allocatable     :: lon_reg
       real(kind=8),  dimension(:)                    :: latval,lonval
       real(kind=8),  dimension(:),   allocatable     :: wts
       real(kind=8),  dimension(:),   allocatable     :: values
       real(kind=8),  dimension(:,:), allocatable     :: series
       logical,       dimension(:),   allocatable     :: used
       character(len=8)                               :: dumpfilename, polyfilename

       real                                           :: fx,fy ! fx = km per degrees LON
       common /scale/fx,fy! fy = km per degrees LAT 

       integer                                        :: npt, npoly
       integer,       dimension(:),   allocatable     :: nstn(maxarea)
       integer,       dimension(:),   allocatable     :: basin(maxarea)
       integer,       dimension(:),   allocatable     :: stnnr, stnwr
       integer,       dimension(:,:), allocatable     :: stndata

       integer                                        :: torigin, torigin_default
       parameter(torigin_default=19500101)
       integer,       dimension(:),   allocatable     ::  times
       real(kind=8),  dimension(:),   allocatable     ::  average        ! weighted average over the entire area 
       integer, dimension(:), allocatable             ::  npt_tot        ! total number of interpolation gridpunts over the entire area 
       integer                                        ::  sumnpt         ! totaal number of fine-grid points over the entire area 
       integer,       dimension(:), allocatable       ::  dates          ! dates for time levels 
       integer                                        ::  istn, cstn, ipt, ipoly, itime
       real(kind=8),  dimension(2,:), allocatable     ::  gridpt
       real(kind=8)                                   ::  value 
       real(kind=8),  dimension(:), allocatable       ::  areal(0:maxarea)
                                        ! for a single day  
       ...
       real(kind=8)                                   ::  xg,yg,xmin, ymin, xmax, ymax      ! xmin, ymin, xmax, ymax 
       real(kind=8),  dimension(:), allocatable       ::  x_int(maxintersect)         ! intersectionpoints 
       real(kind=8)                                   ::  xx,x1,x2,y1,y2              ! temporary intersection point 
       integer                                        ::  n_int                    ! number of intersections at an y-level 
       real(kind=8)                                   ::  delta_lon,delta_lat
       real(kind=8)                                   ::  delta_x,delta_y             ! grid spacing in km 
       real(kind=8)                                   ::  dist,distance,mindist
       parameter (delta_x=0.6,delta_y=0.6)      ! according to belgian data 
       real(kind=8)                                   ::  r_e,r_p,pi,theta0
       ...
       real(kind=8),  dimension(2,:), allocatable     ::  node(2,maxnodes)            ! nodes of a pgon 
       integer                                        ::  nnode                    ! number of node in pgon
       real(kind=8)                                   ::  stnwt_sum                   ! keeps track of the sum of station weights for each sub basin 
       integer                                        ::  inode
      
       integer                                        ::  firstchar, lastchar 
       character(len=20)                              ::  dbuser, dbhost, dbname, dbpass 
       character(len=300)                             ::  datqry 
       integer index2date
...
!      OPEN NETCDF DATA 
       ...
       if(.not.argstring('-ext', trim(fieldname), ext)) then 
          if (verbose) write(0,*) 'No extension provided'
       endif 

       call handle_error ( nf_open(trim(ncfile),nf_nowrite, ncid), '', 'Opening file '//trim(ncfile))
       write(0,'(a)') 'nc-file = '//trim(ncfile)

       call handle_error ( nf_inq_varid (ncid, trim(fieldname), field_varid), '',trim(fieldname))
       ierror = nf_inq_vardimid (ncid, field_varid, field_dimids) 

       if (verbose) write (0,*) trim(fieldname)//' varID = ', field_varid
       if (verbose) write (0,*) 'Dimensions:'
       do idim=1,3
          ierror = nf_inq_dim(ncid, field_dimids(idim), dimname, field_dimlen(idim))
          if (verbose) write(0,'(a,i4,a,i0,a)') '    ', field_dimids(idim),': '//trim(dimname)//' [',field_dimlen(idim),']' 
       enddo 
       ngd1 = field_dimlen(1)
       ngd2 = field_dimlen(2)
       ...
       call handle_error ( nf_inq_varid (ncid, trim(timname), time_varid), '',trim(timname))
       if (verbose) write (0,*) 'Time  varID = ', time_varid
       call handle_error ( nf_inq_varid (ncid, trim(latname), lat_varid), '', trim(latname))
       if (verbose) write (0,*) 'Lat  varID = ', lat_varid
       call handle_error ( nf_inq_varid (ncid, trim(lonname), lon_varid), '', trim(lonname))

       call handle_error (nf_get_att_text (ncid,time_varid,'units',time_units), '', '')
...
! should be 0.1 for EOBS precip. and 0.01 for EOBS temp.
       scale_factor=-1.0
       ierror=nf_get_att_real(ncid,field_varid,'scale_factor', scale_factor)
       if (scale_factor.gt.0) then 
         if (verbose) write(0,*) 'Found scalefactor : ', scale_factor 
       else 
         scale_factor=0.1d0
         if (verbose) write(0,*) 'No scalefactor  found, defaulting to ', scale_factor 
       endif 
        
       ierror=nf_get_att_int(ncid,field_varid,'_FillValue', FillValue)
        if (verbose) write(0,*) 'Fillvalue = ',FillValue 

       call handle_error(nf_get_var_double(ncid, lat_varid, lat_reg),'','') !JB20190430
       call handle_error(nf_get_var_double(ncid, lon_varid, lon_reg),'','') !JB20190430
       call handle_error(nf_get_var_int(ncid, time_varid, times),'','')

!------JB vul nu de (benodigde) arrays lats en lons met resp. de lat_reg en lon_reg waarden

       do iy=1, 201
          do ix=1, 464
             lons((iy-1)*464 + ix) = lon_reg(ix)
             lats((iy-1)*464 + ix) = lat_reg(iy)
          enddo
       enddo
       
!-------SAMPLE SHOWING HOW TO EXTRACT DATA FOR A STATION FROM THE NETCDF-FILE
!       istn=300
!       start(1)=mod(istn-1,ngd1)+1
!       start(2)=(istn-1)/ngd1+1
!       start(3)=1
!       cnt(1)=1
!       cnt(2)=1
!       cnt(3)=1
!           ierror = nf_get_vara_real(ncid, lat_varid,start,cnt,latval)
!           ierror = nf_get_vara_real(ncid, lon_varid,start,cnt,lonval)
!       write(*,*) latval,lonval
!       write(*,*) lats(istn),lons(istn)
!-------SAMPLE SHOWING HOW TO EXTRACT DATA FOR A STATION FROM THE NETCDF-FILE
       ...
       if (verbose) then
           write(0,'(a,f6.2,a2,f6.2,a)') ' grid resolution : ' ,delta_x,' x',delta_y,' km' 
       endif
          ...
          read(line_of_data,*) polynr,partnr,xmin,ymin,xmax,ymax,nnode  
             if (oldpolynr.ge.0) then
                ...
                do ipt=1,npt 
                   mindist=1e+07 
                   do istn=1,ngd1*ngd2
                      dist=(fx*(gridpt(1,ipt)-lons(istn)))**2. + (fy*(gridpt(2,ipt)-lats(istn)))**2. 
                   ...
                   write(34,*) 
                   write(34,'(i5,2e20.7)') polynr,lons(cstn),lats(cstn)
                   write(34,'(i5,2e20.7)') polynr,gridpt(1,ipt),gridpt(2,ipt)
                   write(66,'(2e20.7)')    gridpt(1,ipt),gridpt(2,ipt)
                enddo 
      
                ! add stations to the list of stations for this area 
                nstn(ipoly)=0
                !write(0,*) 'ngd1, ngd2 ', ngd1, ngd2
                do istn=1,ngd1*ngd2
                ...
                do istn=1,nstn(ipoly)
                   write(35,'(2i6,f10.7)') istn,stnnr(istn,ipoly), stnwt(istn,ipoly)
                enddo 
                write(35,*)
      
                ! WRITE SERIES DATA 
                write(fnout,'(a,i3.3,a)') 'area', ipoly, '.'//ext(firstchar(ext):lastchar(ext))
                if (series_append) then
                   open(23,file=fnout(1:instr(fnout,' ')-1), access='APPEND')
                else
                ...
                do istn=1,nstn(ipoly)
                   ...
                   cnt(2)=1
                   cnt(3)=ntime
                   ierror = nf_get_vara_int(ncid, field_varid,start,cnt,stndata(1,istn))
                ...
                do itime=1,ntime 
                   ...
                   do istn=1,nstn(ipoly)
                      ...
                      value=value+stnwt(istn,ipoly)*max(stndata(itime,istn)*scale_factor,-huge(scale_factor))
                   enddo 
 ...
 384               continue 
             ...
             if (verbose) then 
                 write(0,'(a,i4.4,a,i4,a$)') 'Reading definitions of area ', polynr,', part ', partnr + 1, '' 
             else
             ...
             if (verbose) then
                 write(0,'(a,a4,a,i4,a$)') '                            ', '   ',', part ', partnr + 1, '' 
             endif
          endif
          if (verbose) then
 ...
 666   continue
       ...
       do itime=1,ntime 
          if(npt_tot(itime)*1.0/sumnpt.ge.0.8) then 
             write(44,'(i10,f10.3)') dates(times(itime)), average(itime)/npt_tot(itime)
          else 
...
!JB>
       ...
       the1=lat1*pi/180.
       the2=lat2*pi/180.
       distance = r* acos(cos(the1)*cos(the2)*cos(phi2-phi1) + sin(the1)*sin(the2))
       return 
       end 
        ...
        Subroutine handle_error (ierror,pre,post)
          ...
          integer ierror
          character*(*) pre, post
          if (ierror.ne.nf_noerr ) then write(0,*) pre//' '//trim(nf_strerror(ierror))//' : '//post
