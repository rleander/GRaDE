...
!JB    parameter (maxstn=300*300)       ! maximum number of stations #JB tm EObs v17: 272x214
       parameter (maxstn=201*464)       ! maximum number of stations #JB voor EObs v19e, 0.25deg: 201x464
!JB    parameter (maxstn=465*705)       ! maximum number of stations #JB voor EObs v19e, 0.10deg: 465x705
...
!JB    real*4 lats(maxstn)
!JB    real*4 lons(maxstn)
       real*8 lats(maxstn)  !JB20190418
       real*8 lons(maxstn)  !JB20190418
       real*8 lat_reg(201)  !JB20190430 Voor EObs v19e, 0.25deg
       real*8 lon_reg(464)  !JB20190430 Voor EObs v19e, 0.25deg
!JB    real*8 lat_reg(465)  !JB20190430 Voor EObs v19e, 0.10deg
!JB    real*8 lon_reg(705)  !JB20190430 Voor EObs v19e, 0.10deg
...
!JB    call handle_error(nf_get_var_real(ncid, lat_varid, lats),'','')
!JB    call handle_error(nf_get_var_real(ncid, lon_varid, lons),'','')
!JB    call handle_error(nf_get_var_double(ncid, lat_varid, lats),'','')    !JB20190418
!JB    call handle_error(nf_get_var_double(ncid, lon_varid, lons),'','')    !JB20190418
...
!JB       write(161, "(10f7.2)") lons
!JB       write(162, "(10f7.2)") lats
...
!JB<
     ...
     &                    +(fy*(gridpt(2,ipt)-lats(istn)))**2. 
                   ...
                   write(34,*) 
                   write(34,'(i5,2e20.7)') 
     &                       polynr,lons(cstn),lats(cstn)
                   write(34,'(i5,2e20.7)') 
     &                       polynr,gridpt(1,ipt),gridpt(2,ipt)
                   write(66,'(2e20.7)') 
     &                       gridpt(1,ipt),gridpt(2,ipt)
                enddo 
