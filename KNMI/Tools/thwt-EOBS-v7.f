...
C	OPEN NETCDF DATA 
	...
	if(.not.argstring('-var','rr',fieldname)) then 
	...
	write(0,*) fieldname
     ...
     &        , nf_nowrite, ncid)
       ...
       write (0,*) ndim,' dimensions, ', nvar,' variables'
       write (0,*)
       ...
       ierror = nf_inq_dimid (ncid, 'latitude', lat_dimid)          ! time ID
       ierror = nf_inq_dim(ncid, lat_dimid, name, nlat)         ! inquire t leve
       write (0,*) 'Lat dimID  = ', lat_dimid,'  ',nlat,' levels'

       ierror = nf_inq_dimid (ncid, 'longitude', lon_dimid)          ! time ID
       ...
       ierror = nf_inq_varid (ncid, 'Actual_latitude', lat_varid)
       write (0,*) 'Lat  varID = ', lat_varid
       ierror = nf_inq_varid (ncid, 'Actual_longitude', lon_varid)
     ...
     &      FillValue)
       ...
       ierror = nf_get_var_int(ncid, time_varid, times)
...
C     &		         lons(istn),lats(istn),r_e) 
     ...
     &	                               stnwt(istn,ipoly)
	   ...
	   ! WRITE SERIES DATA 
	   write(fnout,'(a,i3.3,a)') 'area',ipoly,'.rr'
!	   write(fnout,'(a,i3.3,a)') 'area_e',ipoly,'.'
